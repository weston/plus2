...
type ZenPhase = 'idle' | 'inspecting' | 'solving' | 'done';
...
export default function ZenPage() {
  ...
  // Zen = optional clock. Hidden timer still records times quietly.
  const [showTimer, setShowTimer] = useState(true);
  useEffect(() => {
    setShowTimer(localStorage.getItem('plus2-zen-timer') !== 'off');
  }, []);
  const toggleTimer = () => {
    setShowTimer((v) => {
      localStorage.setItem('plus2-zen-timer', v ? 'off' : 'on');
      return !v;
    });
  };
  const [times, setTimes] = useState<SolveTime[]>([]);
  const animationSpeed = useCubePrefs((st) => st.speed);

  const moveSeqRef = useRef(0);
  const inspectionTimerRef = useRef<NodeJS.Timeout | null>(null);
  const phaseRef = useRef<ZenPhase>('idle'); // Ref for current phase to avoid closure issues
  ...
  const stopTimer = useCallback(async () => {
    ...
    // Close the input gate BEFORE the async solved check — a keypress in that
    // window used to register an extra move and leave the "finished" cube one
    // or two turns off.
    setPhase('done');
    ...
    // Trim any accidental trailing inputs so the cube displays (and records)
    // the exact finishing state.
    let finalMoves = moves;
    if (isSolved) {
      const n = cubeRef.current?.getSolvedMoveCount();
      if (typeof n === 'number' && n < moves.length) {
        finalMoves = moves.slice(0, n);
        setMoves(finalMoves);
      }
    }
    ...
    const newTime: SolveTime = {
      time,
      scramble,
      moveCount: countNonRotationMoves(finalMoves),
      timestamp: new Date(),
      dnf: !isSolved,
    };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <header className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold">Zen Mode</h1>
          </div>
            ...
            <div className="card">
              <div className="flex flex-col items-center">
                ...
                <div className="mb-6 text-center">
                  ...
                  {showTimer && phase === 'solving' && (
                    <Timer
                      startTime={solveStart}
                      isRunning={true}
                    />
                  )}
                  {showTimer && phase === 'done' && solveTime !== null && (
                  ...
                  <p className="text-gray-400 mt-4 text-sm md:text-base">
                    ...
                    {phase === 'inspecting' && 'Inspecting... Make a move to begin'}
                    {phase === 'solving' && 'Solving... stops automatically when solved'}
