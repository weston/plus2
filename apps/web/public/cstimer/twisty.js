 ...
 *
 * Vendored from csTimer (https://github.com/cs0x7f/cstimer), GPL-3.0.
 * MODIFIED for plus2: removed the jQuery dependency, the csTimer touch-input
 * table, and the `kernel` coupling (config now comes from a small CSTCFG shim).
 * These modifications are also GPL-3.0; see public/cstimer/NOTICE.
 */
"use strict";
...
window.twistyjs = (function() {
	...
	twistyjs.TwistyScene = function() {
		 ...
		 */
		var twistyContainer = document.createElement('div');
		twistyContainer.style.width = '100%';
		twistyContainer.style.height = '100%';
		twistyContainer.style.position = 'relative';
		...
		this.initializeTwisty = function(twistyType) {
			...
			// (csTimer touch-input table removed; plus2 drives moves via its own keybindings)
		...
		this.resize = function() {
			...
			// to the desired size.
			var min = Math.min(twistyContainer.clientWidth, twistyContainer.clientHeight);
			// plus2: shrink the canvas below the container size to leave background margin.
			var fit = parseFloat(CSTCFG.getProp('vrcFit', '1')) || 1;
			min = Math.round(min * fit);
			camera = new THREE.Camera(30, 1, 0, 1000);
			var ori = CSTCFG.getProp('vrcOri', '6,12');
			ori = ori.split(',');
		...
		*/
		...
		this.addMoves = function(moves, ts) {
			...
			movets.forEach(fireMoveAdded);
			if (~~CSTCFG.getProp('vrcSpeed', 100) == 0) {
		...
		function animateLoop(timeStamp) {
			timeStamp = Date.now();
			var timeProgress = (timeStamp - lastTimeStamp) / (CSTCFG.getProp('vrcSpeed', 100) || 1e-3) * (moveQueue.length + 2) / 2;
